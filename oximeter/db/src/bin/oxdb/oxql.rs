                ...
                match cmd {
                    ...
                    _ => {
                        if let Some(timeseries_name) = cmd.strip_prefix("\\d") {
                            ...
                            {
                                Ok(result) => {
                                    if opts.print_elapsed {
                                        print_elapsed_duration(
                                            result.total_duration,
                                        );
                                    }
                                    if opts.print_summaries {
                                        print_summary(&result.query_summaries);
                                    }
...
fn print_elapsed_duration(total_duration: Duration) {
    println!("{}: {:?}\n", "Total duration".underlined().bold(), total_duration)
}

fn print_summary(summaries: &[QuerySummary]) {
    println!("{}", "SQL queries".underlined().bold());
