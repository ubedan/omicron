...
impl Query {
    ...
    // Check that all subqueries (grouped table ops) are followed by a merging
    // operation.
    pub(crate) fn merge_ops_follow_subqueries(&self) -> bool {
        ...
        while let (Some(op), maybe_next_op) = (ops.next(), next_ops.next()) {
            if let TableOp::Grouped(GroupedTableOp { ops: subq_ops }) = op {
                ...
                if !subq_ops.iter().all(Query::merge_ops_follow_subqueries) {
                    return false;
                }
            }
        }
    ...
    pub(crate) fn split(&self) -> SplitQuery {
        match &self.ops[0] {
            TableOp::Basic(BasicTableOp::Get(_)) => {
                SplitQuery::Flat(crate::oxql::Query { parsed: self.clone() })
            }
            TableOp::Basic(_) => unreachable!(),
            TableOp::Grouped(GroupedTableOp { ops }) => SplitQuery::Subquery {
                subqueries: ops
                    .iter()
                    .cloned()
                    .map(|parsed| crate::oxql::Query { parsed })
                    .collect(),
...
// Either a flat query or one with subqueries.
//
