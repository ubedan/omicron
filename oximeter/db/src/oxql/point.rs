...
impl ValueArray {
    ...
    fn as_boolean_mut(&mut self) -> Result<&mut Vec<Option<bool>>, Error> {
    ...
    fn as_integer_mut(&mut self) -> Result<&mut Vec<Option<i64>>, Error> {
        let ValueArray::Integer(inner) = self else {
            anyhow::bail!(
                "Cannot access value array as integer type, it has type {}",
                self.data_type(),
    ...
    fn as_string_mut(&mut self) -> Result<&mut Vec<Option<String>>, Error> {
        let ValueArray::String(inner) = self else {
            anyhow::bail!(
                "Cannot access value array as string type, it has type {}",
                self.data_type(),
    ...
    fn as_integer_distribution_mut(
        &mut self,
    ) -> Result<&mut Vec<Option<Distribution<i64>>>, Error> {
        let ValueArray::IntegerDistribution(inner) = self else {
            anyhow::bail!(
