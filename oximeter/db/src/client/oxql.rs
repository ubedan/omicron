...
impl Client {
    ...
    pub async fn oxql_query(
        ...
        let result = self.run_oxql_query(parsed_query).await;
    ...
    async fn run_oxql_query(
        ...
        if let oxql::ast::SplitQuery::Subquery { subqueries, transformations } =
            split
        {
            trace!(
                &self.log,
            ...
            for subq in subqueries.into_iter() {
                let res = self.run_oxql_query(subq).await?;
            ...
            for tr in transformations.into_iter() {
                ...
                let new_tables = tr.apply(&tables);
        ...
        // that apply to this timeseries in particular.
        let preds = query.coalesced_predicates();
        debug!(
            &self.log,
